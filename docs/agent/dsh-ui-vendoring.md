...
> **状态：映射已按实测回填（哈希三列核对，锚点 `70d7342` = 本切片在 **main** 上的 squash 合并提交（squash 前的实现线冻结点为 `069b2f1`，两者 vendored 子树 blob **逐字节相同**，已实测核对））。** 首批 L1+L2 切片已提交在 `feat/p1-138-vendor-ui-primitives` 分支：`ee9c4b5` 落地 vendored 子树与 L1 token → `f3452f0` 补 Q5 判据 → `8490228`（AA 重映射 + 出处头改①/②分类）→ **`069b2f1`（修正两处计数）** → 已 squash 合并进 main（`70d7342`，PR #151）。第 3 节的路径、**三列 blob SHA**、字节数与改动分类均**实测自锚点 `70d7342`（= `069b2f1` 的内容）的提交内容**（`git rev-parse <commit>:<path>` + 按注释块剥出处头 + diff，方法见 §3.4），不是从任务描述或工作区快照推测的。
>
...
> **复算口径（必须写清，否则数字对不上）**：三列都先**剥注释**再数——文件头与段注释里写了变量名来解释"为什么删/为什么留"，不剥注释会把说明文字当成声明（#159 实测踩过：同一命令在剥/不剥两种口径下能差出 20 多个）。相加关系：**57 = 38 + 19**（38 个被 vendored CSS 引用，19 个只被 alias 段与应用层消费，两集无交集，`comm -23` 实测）。
...
> **两个提交号的关系（2026-09-11 追记）**：`069b2f1` 是切片在 `feat/p1-138-vendor-ui-primitives` 上的**分支头（冻结点）**，`70d7342` 是它 **squash 合并进 main 后的提交号**（PR #151）。squash 会改写提交号，但**内容相同**——已实测 `Button.tsx`/`Tag.tsx`/`StateDot.module.css`/`manifest.json` 四个 blob 两处逐字节一致，故下表的哈希**无需重算**。复算时：查 main 用 `C=70d7342`，查分支历史用 `C=069b2f1`（GitHub 保留 `refs/pull/151/head`，仍可取）。
>
> **哈希锚点（读表前必读）**：下表三个哈希列全部对应 **vendored 子树在 main 上的提交状态 `70d7342`**（PR #151 的 squash 合并提交；其前身分支冻结点 `069b2f1` 内容相同；`manifest.json` 与 Q0 单测也钉在这个状态上）。复算用 `git rev-parse <commit>:<path>` 或 `git show <commit>:<path> | git hash-object --stdin`——**不要**对**工作区**跑 `git hash-object`：分支历史上多次出现"工作区值 ≠ 提交值"，本台账早期版本正因此记下整列错值。**凡本台账的哈希都以"某 commit 的 blob"为单位**，不以下一时刻的工作区为单位。
...
所以本台账用**三列**分开记，不把两种值混在一列：**上游 blob**（钉住 commit 下的原文）、**本仓 blob**（`70d7342` 里实际提交的那份）、**剥出处头后 blob**（本仓文件去掉出处头块后的内容哈希，用于判断"除出处头外还改了什么"）。

**`.module.css`（6/6：剥掉出处头后与上游逐字节一致）**

| # | 组件 | 上游 blob | 本仓 blob@`70d7342` | 剥头后 blob | 头行数 | 剥头后 vs 上游 |
|---|---|---|---|---|---|---|
...
| # | 组件 | 上游 blob | 本仓 blob@`70d7342` | 剥头后 blob | 头行数 | 剥头后 vs 上游 | 剥头后仍然存在的改动 |
|---|---|---|---|---|---|---|---|
...
**改动分类与实现线出处头逐字对齐**（`70d7342` 的最终版）：出处头把每处改动归入两类，本台账右列沿用同一口径——
...
**同目录支撑文件（一并被取用，`manifest.json` 已登记）——blob 均为 `70d7342` 的提交值**：

| 本仓文件 | 本仓 blob@`70d7342` | 字节 | 上游来源 | 关系 |
|---|---|---|---|---|
...
- 2026-09-11：锚点由分支冻结点 `069b2f1` 前移到 **main 的 squash 合并提交 `70d7342`**（PR #151 已合入）。squash 会改写提交号，故锚点必须跟着落到 main 上；实测核对 `Button.tsx`/`Tag.tsx`/`StateDot.module.css`/`manifest.json` 四个 blob 与 `069b2f1` **逐字节相同**，三列哈希无需重算，仅换标签。
