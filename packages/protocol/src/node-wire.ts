 ...
 * 命名：`run.followup` 在本地 Node↔Runtime wire（runtime-wire.ts）里已占用平名
 * `RunFollowupSchema`，故本帧按 node-wire 既有避让惯例（NodeHello/NodeInventory/
 * NodeTokenRevoked 同例）加 `Node` 前缀。
 *
 ...
 * （切片③）。
 */
export const NodeRunFollowupSchema = envelope(
  'run.followup',
  z.strictObject({
    commandId: z.uuid(),
    runId: z.uuid(),
