...
| 门 | 命令 | 目的 | 通过标准 |
|---|---|---|---|
| Q0 静态门 | `pnpm check` | format、lint、类型（**含 `tests/` 的 typecheck**，#178）、依赖边界（含「有 tests/ 必配 typecheck」护栏）、协议生成漂移 | 零错误，生成物无 diff |
...
| Q8 性能门 | `pnpm test:load` | 10 浏览器连接、2 并发 Run | 达到 p95 门槛，无持续内存增长（机检口径=空闲段 RSS 留存斜率，判据环境 4CPU/8GiB/Linux，细则见 load-performance-acceptance） |
