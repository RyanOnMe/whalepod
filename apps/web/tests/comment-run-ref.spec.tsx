...
describe('讨论列表里的引用 chip（页面级）', () => {
  it('点引用 chip 打开那次运行的 Console', async () => {
    ...
    const activeRun = makeRun({ id: RUN_A, status: 'running' })
    renderApp(
      `/tasks/${task.id}`,
      loggedInHandlers(BOB, [
          ...
          respond: () =>
            new Response(
              JSON.stringify({
                ...
                data: {
                  task,
                  comments: [makeComment({ body: `按 ${formatRunReference(RUN_A)} 的结果继续` })],
                  instructions: [],
                  runs: [activeRun],
                  artifacts: [],
                },
        ...
        {
          method: 'GET',
          url: new RegExp(`/api/v1/runs/${RUN_A}$`),
          respond: () =>
            new Response(
              JSON.stringify({ ok: true, data: { ...activeRun, projectId: task.projectId } }),
              { status: 200 },
            ),
        },
        {
    ...
    const chip = await screen.findByTestId('comment-run-ref')
    expect(chip).toHaveTextContent('运行 R-aaaaaaaa')
    await user.click(chip)
    // 打开的是**这一次**运行的 Console（不是随便哪一次）
