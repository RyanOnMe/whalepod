/**
 * 中间区域：Run 状态时间线 + 审批卡片插槽（02 Task 7 Step 4）。
 * Run 数据来自 Task Room 聚合视图；审批卡片待实时事件链路（P1-08）落
 * HTTP/WS 后接通，这里只预留插槽并说明，不伪造审批数据。
 */
...
export function RunTimeline({ runs }: RunTimelineProps): ReactNode {
  if (runs.length === 0) {
    return (
      <p className="empty-state">
        还没有 Run。接受任务并创建 Run 后，执行历史会出现在这里（Run 创建随后续版本提供）。
      </p>
    )
  }
  return (
