/**
 * vendored from deepseek-ai/deepseek-harness packages/client/ui-primitives/src/Menu.tsx
 * @ c291e7961a515f6d7af9304e7fd1d257929aef26（MIT，见同目录 LICENSE）
 * 本仓改动分两类：①工程口径（`clsx` → `./cx.js`（6 处调用）；图标 import 改指本目录
 * `./icons.js`、hook import 改指 `./pointer-grace.js`（两者本目录都已 vendored）；
 * 三个上游 import 都带 `.tsx`/`.ts` 后缀，本仓按 NodeNext 口径改成 `.js`）；
 * ②可测性（wrapper span 新增根属性 `data-vendored="menu"`、允许调用方覆写 `data-testid`）。
 *
 * 可移植性结论（挑选时实测，详见 docs/agent/dsh-ui-vendoring-batch2.md）：Menu **不需要**
 * 裁剪就能整取——零 cordis、零 DSH 包依赖、零 schemastery，运行时依赖只有 react /
 * react-dom / 本目录的 pointer-grace 与 icons。它是本批最大的文件（14318 B），
 * 也是**跟版成本最高**的一个（上游清单 §4.2 已把它列为交互细节频繁变动项）。
 *
 * 迁移页面时必须知道的三点（都是上游行为，本仓未改）：
 *   1. 它是**受控**组件：`open` 由调用方持有，`onSelect`/`onClose` 只上报意图；
 *      Menu 自己不写 open。
 *   2. `anchor` 是 render prop 位置的节点（Menu 包一层 `<span class="root">`），
 *      portal=false 时列表用纯 CSS 相对定位、贴着这个 wrapper；**若祖先有 overflow
 *      裁剪，必须开 `portal`**，此时定位改由 anchor rect 算（必要时用 `getAnchorRect`
 *      直接给矩形，避免与宿主布局 effect 竞态）。
 *   3. 列表整体在 portal 模式下渲染到 `document.body`（React 树里仍在 Menu 下，
 *      合成事件会冒泡回锚点行，故列表根上有一个 `stopPropagation`）。
 * 与原生 `<select>` 的对应关系：`items` 里的普通项对应 `<option>`，
 * `type:'label'` 对应 `<optgroup label>` 的标题、`type:'separator'` 是 `<hr>`
 * 一类的东西（原生 select 没有对应物），`selectedId` 对应 value。
 */
import { useEffect, useLayoutEffect, useRef, useState } from 'react'
import type { CSSProperties, ReactNode } from 'react'
import { createPortal } from 'react-dom'
import { cx } from './cx.js'
import { IconCheckOutline16 } from './icons.js'
 ...
 */
export function Menu({ open, anchor, items, selectedId, selectedIds, onSelect, onClose, align = 'start', side = 'bottom', portal = false, closeOnPointerLeave = false, dense = false, compact = false, autoFocus = false, selection = 'check', getAnchorRect, footer, className, 'data-testid': dataTestId }: {
  open: boolean
  autoFocus?: boolean
  anchor: ReactNode
    ...
    return (
      ...
      >
        <button
          type="button"
          role="menuitem"
          className={cx(css.item, selected && (selection === 'fill' ? css.selectedFill : css.selected), entry.danger === true && css.danger)}
        ...
        {subOpen && entry.submenu !== undefined && (
          <div className={cx(css.submenu, compact && css.compactList)} role="menu">
  ...
  const list = open && (
    <div
      ref={listRef}
      className={cx(css.list, dense && css.denseList, compact && css.compactList, scrollable && css.scrollable, portal && css.portal, side === 'top' && !portal && css.sideTop, align === 'end' && !portal && css.alignEnd)}
