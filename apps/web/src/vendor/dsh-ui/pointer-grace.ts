/**
 * vendored from deepseek-ai/deepseek-harness packages/client/ui-primitives/src/pointer-grace.ts
 * @ c291e7961a515f6d7af9304e7fd1d257929aef26（MIT，见同目录 LICENSE）
 * 本仓改动：仅①工程口径（顶部加本出处头块）。**代码与上游逐字节一致**——
 * 剥掉出处头后与上游 blob 相等（见 docs/agent/dsh-ui-vendoring-batch2.md §哈希表）。
 * 它是 Menu 的**同目录运行时依赖**（`usePointerGrace`），本身零 cordis、只 import react；
 * 首批没取是因为首批 6 个原语里只有 Menu/HoverCard 用得上它。
 */
