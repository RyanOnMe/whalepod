 ...
 *
 * 成因：`apps/node/package.json` 声明 `bin.whalepod-node → ./dist/cli.js`，但
 * `src/cli.ts` 只 `export async function main(argv)`、**文件结尾没有顶层调用**，
 * 于是装好的 `whalepod-node <任意子命令>` 加载模块、定义函数、exit 0 静默退出。
...
function declaredBin(): string {
  ...
  const target = pkg.bin?.['whalepod-node']
  if (target === undefined) throw new Error('package.json 未声明 bin.whalepod-node')
...
describe('#95 whalepod-node 二进制必须真的执行 main', () => {
  it('无参数执行：输出 usage 并 exit 2（而非静默 exit 0）', async () => {
    const res = await runCli([])
    expect(res.stderr).toContain('usage: whalepod-node')
  ...
  it('作为库 import 时不得自动执行 main（顶层调用的保护条件）', async () => {
    ...
    expect(res.stderr).not.toContain('usage: whalepod-node')
