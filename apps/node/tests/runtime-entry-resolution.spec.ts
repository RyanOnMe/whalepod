 ...
 * Runtime 入口的包解析契约（#97）。
 *
 * 成因：`apps/node/src/cli.ts` 用 `createRequire(...).resolve('@whalepod/runtime/dist/bin.js')`
 * 找 Runtime 子进程入口，而 `apps/runtime/package.json` 的 `exports` 只声明了 `"."`
 * ——深路径解析被 Node 以 ERR_PACKAGE_PATH_NOT_EXPORTED 拒绝，`whalepod-node start`
...
const RUNTIME_BIN_SPECIFIER = '@whalepod/runtime/bin'
...
describe('#97 Runtime 入口必须按 exports 公开子路径可解析', () => {
  it('exports 子路径与 package.bin 指向同一真实文件', () => {
    ...
    const binRel = pkg.bin?.['whalepod-runtime']
    expect(binRel, 'runtime 包必须声明 bin.whalepod-runtime').toBeDefined()

    // 从 @whalepod/node 的模块上下文解析（与 cli.ts 同一依赖视角）。
  ...
  it('生产 cli 用的就是这个说明符（一致性锁，防漂移）', () => {
    ...
    expect(source).not.toContain('@whalepod/runtime/dist/bin.js')
  })
})
