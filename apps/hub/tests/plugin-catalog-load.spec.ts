...
function makeManifest(overrides: ManifestOverrides = {}): Record<string, unknown> {
  const name = overrides.name ?? 'p311-load'
  return {
    schemaVersion: 1,
    name,
    version: overrides.version ?? '1.0.0',
    tarballUrl: `https://catalog.fixtures.project311.test/tarballs/${name}.tgz`,
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'p311-catalog-load-'))
  tempDirs.push(dir)
  return dir
}
...
describe('loadPluginCatalog fail-closed（P1-17 M7）', () => {
  ...
  it('非法 JSON manifest → rejects，错误点名 catalog 相对文件名', async () => {
    const dir = await seedCatalog({
      'catalog/p311-broken.json': '{ "schemaVersion": 1, ',
      'locks/p311-broken@1.0.0.lock.yaml': 'schemaVersion: 1\n',
    })
    await expectCatalogReject(dir, /^plugin catalog 文件不是合法 JSON：catalog\/p311-broken\.json$/)
  })

  it('schema 非法（version 用 range ^1.0.0）→ rejects，点名字段与原因', async () => {
    const dir = await seedCatalog({
      'catalog/p311-range.json': JSON.stringify(
        makeManifest({ name: 'p311-range', version: '^1.0.0' }),
        null,
        2,
      ),
      'locks/p311-range@1.0.0.lock.yaml': 'schemaVersion: 1\n',
    })
    await expectCatalogReject(
      dir,
      /^plugin catalog 清单校验失败：catalog\/p311-range\.json —— version version must be exact semver/,
    )
  })

  it('两个文件重复登记同 name@version → rejects（按文件序点名后者）', async () => {
    const dir = await seedCatalog({
      'catalog/p311-dup-a.json': JSON.stringify(makeManifest({ name: 'p311-dup' }), null, 2),
      'catalog/p311-dup-b.json': JSON.stringify(makeManifest({ name: 'p311-dup' }), null, 2),
      'locks/p311-dup@1.0.0.lock.yaml': 'schemaVersion: 1\n',
    })
    await expectCatalogReject(
      dir,
      /^plugin catalog 重复登记：p311-dup@1\.0\.0（catalog\/p311-dup-b\.json）$/,
    )
  })

  it('manifest 存在但对应 lock 缺失 → rejects，点名缺失的 lock 相对路径', async () => {
    const dir = await seedCatalog({
      'catalog/p311-nolock.json': JSON.stringify(makeManifest({ name: 'p311-nolock' }), null, 2),
      // 故意只放别的包的 lock：p311-nolock@1.0.0.lock.yaml 缺失。
      'locks/p311-other@1.0.0.lock.yaml': 'schemaVersion: 1\n',
    })
    await expectCatalogReject(
      dir,
      /^plugin catalog 不完整：locks\/p311-nolock@1\.0\.0\.lock\.yaml 缺失（catalog\/p311-nolock\.json 声明了该包）$/,
    )
  })

  it('lock 文件为空（纯空白）→ rejects', async () => {
    const dir = await seedCatalog({
      'catalog/p311-emptylock.json': JSON.stringify(
        makeManifest({ name: 'p311-emptylock' }),
        null,
        2,
      ),
      'locks/p311-emptylock@1.0.0.lock.yaml': '   \n\t\n',
    })
    await expectCatalogReject(
      dir,
      /^plugin catalog lock 文件为空：locks\/p311-emptylock@1\.0\.0\.lock\.yaml$/,
    )
  })
  ...
  it('catalog 目录整体缺失 → resolves 为空 catalog（dirMissing=true，安装面关闭）', async () => {
    ...
    expect(catalog.get('p311-load', '1.0.0')).toBeUndefined()
    expect(catalog.lockfile('p311-load', '1.0.0')).toBeUndefined()
  })

  it('catalog 目录存在但无 manifest → 空 manifests 且 dirMissing=false（与目录缺失区分）', async () => {
    const dir = await seedCatalog({ 'locks/p311-ghost@1.0.0.lock.yaml': 'schemaVersion: 1\n' })
  ...
  it('happy path 自证：仓库 fixture catalog 可加载（命中 get/lockfile、稳定排序）', async () => {
    ...
    expect(keys).toContain('p311-echo@0.2.1')
    expect(keys).toContain('p311-fixed-time@0.1.0')
    expect(keys).toEqual([...keys].sort())
    expect(catalog.get('p311-echo', '0.2.1')?.entrypoint).toBe('dist/echo.js')
    expect(catalog.get('p311-echo', '9.9.9')).toBeUndefined()
    expect(catalog.lockfile('p311-echo', '0.2.1')).toContain('p311-echo')
  })
})
