...
Hub 侧对 `command.ack` 的映射（切片③ 接线时必须照此实现）：
`accepted=true` → 指令 `instruction_state=accepted` 并记 `run_id`；`accepted=false` →
`instruction_state=rejected(该错误码)` 并**在 UI 明示**（尤其 `INVALID_RUN_TRANSITION` 与
`RUNTIME_LOST` 两种用户可见语义：Run 已结束 / Runtime 已丢失）。
