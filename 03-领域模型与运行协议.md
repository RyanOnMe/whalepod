...
follow-up turn。受理与否用既有 `command.ack { commandId, accepted, error? }` 回报——
`accepted=true` 的语义是「**已受理并下发到 Runtime stdin**」，不是「模型已读到」；进展仍由
`session.event` / `run.*` 上行帧呈现。Hub 据此把指令的 `instruction_state` 落 `accepted`。

拒绝一律用 §10 现有码、且均不下发：本地已终态 → `INVALID_RUN_TRANSITION`（终态禁止复活）；
本 Node 无该 Run 事实 → `NOT_FOUND`；未到 `running`（queued/dispatching）→
`INVALID_RUN_TRANSITION`（该窗口由 Hub 侧排队，见 ADR-0009 决策 5）；Runtime 已不在 →
`RUNTIME_LOST`。重复 `commandId` 幂等：只回放 ack，绝不把同一句追问注入两次。
