...
RUN pnpm deploy --legacy --filter @project311/hub --prod /deploy/hub

FROM node:24-bookworm-slim
RUN useradd -r -u 10001 project311
WORKDIR /app
COPY --from=build --chown=project311:project311 /deploy/hub /app
# 见头部约束 2：迁移目录实体随镜像走（路径必须落在 @project311/db 的 dist 同级）。
COPY --from=build --chown=project311:project311 /repo/packages/db/migrations /app/node_modules/@project311/db/migrations
# curated 插件 catalog（只读随镜像；缺目录 = 安装面关闭 + 启动告警，见 app.ts）。
COPY --from=build /repo/plugins /app/plugins
RUN mkdir -p /data && chown project311:project311 /data
USER project311
ENV NODE_ENV=production \
    PROJECT311_SETUP_TOKEN_PATH=/data/setup-token \
    PROJECT311_ARTIFACT_STORE_DIR=/data/artifact-store \
    PROJECT311_PLUGIN_CATALOG_DIR=/app/plugins/catalog \
